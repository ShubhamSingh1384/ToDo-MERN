...
app.get('/todo/add', (req, res) =>{
    ...
    .then((todo) =>{
        ...
        res.status(200).send({message: "New Task added successfully", success: true})
        return todo.save();
    })
...
app.post('/todo/signup', (req, res)=>{
    ...
    .then((data) => {
        ...
        if(data === null){
            ...
            newUser.save()
            .then(() => res.status(200).send({message:"User registered successfully", success: true}))
            .catch((error) => res.status(400).send({message: "failed to register user", success: false}))
        }
        else{
            res.status(400).send({message: "user already exist"})
        }
    })
    .catch((error) => res.status(500).send({message: "error in signUp ", error}))
})
